...
import { UserRole, AdminDashboardStatsResponse, ManagedFile, RecentActivity } from "../types";
import {
  fetchAdminDashboardStatsAPI,
  fetchClientDashboardStatsAPI,
  fetchFreelancerDashboardStatsAPI,
  fetchUserRecentFilesAPI,
...
const StatCard: React.FC<StatCardProps> = ({
  ...
  const content = (
    <div
      className={`bg-white p-5 md:p-6 rounded-xl shadow-lg flex items-center space-x-4 hover:shadow-xl transition-shadow duration-200 border-l-4 ${colorClasses[color]}`}
    >
      <div
        className={`p-3 rounded-full ${colorClasses[color].split(" ")[0]} ${colorClasses[color].split(" ")[1]}`}
      >
        {React.cloneElement(icon as React.ReactElement<IconProps>, {
          className: "w-7 h-7 md:w-8 md:w-8"
        })}
      </div>
      <div>
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        if (user.role === UserRole.ADMIN) {
          ...
          const files = await fetchUserRecentFilesAPI();
    ...
    return (
      <>
        <StatCard 
          ...
          value={adminStats.total_active_users} 
          icon={<UsersIcon />} 
          linkTo="/admin/users" 
          color="blue" 
        />
  ...
  return (
    <div className="space-y-8">
      ...
      <p className="text-gray-600">
        Welcome back, {user.name}! Here's a quick look at your platform activity.
      </p>
      ...
      <div className="mt-10 p-6 bg-primary-extralight rounded-xl shadow">
        ...
        <div className="flex flex-wrap gap-3">
          {user.role === UserRole.ADMIN && (
            <>
              <Link to={`${NAV_LINKS.DASHBOARD}/${NAV_LINKS.ADMIN_PROJECTS}`}>
                <button className="bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded-lg text-sm">
                  Manage Projects
                </button>
              </Link>
              <Link to={`${NAV_LINKS.DASHBOARD}/${NAV_LINKS.ADMIN_USERS}`}>
                <button className="bg-secondary hover:bg-primary text-white px-4 py-2 rounded-lg text-sm">
                  Manage Users
                </button>
              </Link>
              <Link to={`${NAV_LINKS.DASHBOARD}/${NAV_LINKS.ADMIN_TIME_REPORTS}`}>
                <button className="bg-secondary hover:bg-primary text-white px-4 py-2 rounded-lg text-sm">
                  View Time Reports
                </button>
              </Link>
          ...
          {user.role === UserRole.FREELANCER && (
            <>
              ...
              <Link to={`${NAV_LINKS.DASHBOARD}/${NAV_LINKS.FREELANCER_JOB_CARDS}`}>
                <button className="bg-secondary hover:bg-primary text-white px-4 py-2 rounded-lg text-sm">
                  My Job Cards
                </button>
              </Link>
          ...
          {user.role === UserRole.CLIENT && (
            <Link to={`${NAV_LINKS.DASHBOARD}/${NAV_LINKS.CLIENT_MY_PROJECTS}`}>
              <button className="bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded-lg text-sm">
                View My Projects
              </button>
            </Link>
          )}
          <Link to={`${NAV_LINKS.PROFILE}`}>
            ...
            {recentActivity.length > 0 ? (
              <ul className="space-y-4 max-h-72 overflow-y-auto">
                {recentActivity.map((activity) => (
                  // Assuming activity has an 'iconName' or similar to map to an IconComponent
                  <li
                    key={activity.id}
                    className="flex items-start space-x-3 pb-3 border-b border-gray-100 last:border-b-0"
                  >
                    <div className="flex-shrink-0 mt-1 text-primary">
                      {activity.iconName === "CheckCircleIcon" ? (
                        <CheckCircleIcon className="w-5 h-5" />
                      ) : activity.iconName === "PencilIcon" ? (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">                        {new Date(activity.timestamp).toLocaleString()}
                      </p>
                    </div>
                  </li>
                ))}
              ...
              {recentFiles.length > 0 ? (
                <ul className="space-y-3 max-h-72 overflow-y-auto">
                  {recentFiles.map((file) => (
                    <li
                      key={file.id}
                      className="flex items-center space-x-3 pb-2 border-b border-gray-100 last:border-b-0"
                    >
                      <img
                        src={getMockFileIconPath(file.type)}
                        alt={file.type}
